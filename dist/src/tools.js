...
const EVIDENCE_ROLES = ["acceptance", "diagnostic"];
...
export function registerTaskTools(pi, store, idGenerator = new SequentialIdGenerator()) {
    ...
    registerGuidedTool(pi, {
        ...
        promptSnippet: "Record successful evidence and advance the current atomic pi-tasks step in one operation",
        promptGuidelines: [
        ...
        execute: async (_toolCallId, params, _signal, _onUpdate, ctx) => {
            ...
            const step = task?.planSteps.find((candidate) => candidate.id === params.step_id);
            const criterionIds = params.criterion_ids ?? step?.criterionIds ?? [];
            const criteriaBelongToStep = criterionIds.every((criterionId) => step?.criterionIds.includes(criterionId));
            const criteriaLinked = duplicate
                ? criterionIds.every((criterionId) => task?.acceptanceCriteria
            ...
            return appendAndReport(pi, store, ctx, event, `Verified and completed step ${params.step_id} with evidence ${evidenceId}`);
        },
    });
    registerGuidedTool(pi, {
        name: "task_decision",
...
function taskEvidenceParametersSchema() {
    return Type.Object({
        task_id: Type.String(),
        type: Type.Enum(EVIDENCE_TYPES),
        role: Type.Optional(Type.Enum(EVIDENCE_ROLES, {
            description: "Evidence purpose. Defaults to acceptance; diagnostic evidence cannot satisfy criteria or task completion.",
        })),
        level: Type.Enum(VERIFICATION_LEVELS),
        summary: Type.String(),
        ...
        supersedes_evidence_ids: Type.Optional(Type.Array(Type.String(), {
            description: "Failed evidence IDs explicitly replaced by this passing evidence.",
        })),
        reason: Type.Optional(Type.String({
            description: "Required non-empty explanation when supersedes_evidence_ids is provided.",
        })),
        quality: evidenceQualitySchema(),
...
function buildEvidenceRetryExample(event) {
    ...
    const artifactRefs = event.evidence.quality?.artifactRefs &&
...
function baseEvent(type, taskId, _ctx, payload) {
